...
import { LayoutDashboard, Users, FileText, Briefcase, CalendarCheck, LogOut, Clock, Home, Receipt, ClipboardList, FileCheck, Shield, UserCog, User } from "lucide-react";
...
const AdminSidebar = ({ onLogout, userId, userName }) => {
  ...
  const patrolItems = [
    { path: "/checkpoints", icon: MapPin, label: "Checkpoints" },
    { path: "/patrol-reports", icon: RouteIcon, label: "Patrol Reports" },
    { path: "/defects", icon: AlertTriangle, label: "Defects" },
  ];
    ...
    <aside className="w-64 min-h-screen bg-white border-r border-border flex flex-col" data-testid="sidebar">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto" data-testid="navigation">
        ...
        <div className="pt-4 mt-4 border-t border-border">
          <p className="px-3 mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Patrol Management</p>
